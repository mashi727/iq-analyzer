"""Main viewer window — orchestrates loaders, panels and the three plots.

The class itself remains substantial because the file-lifecycle, region/ROI
synchronisation and spectrogram-update flow are all genuinely cross-cutting
concerns that span every panel. The pure signal-processing and panel UI have
already been pulled out into :mod:`iq_analyzer.core` and
:mod:`iq_analyzer.widgets`; what's left is the glue that ties them together.
"""
...
# The implementation is preserved as-is during this migration; subsequent
# refactor passes can split the region/ROI orchestration further.
